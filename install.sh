...
for c in openssl tar; do
    command -v "$c" >/dev/null 2>&1 || { echo "[-] 缺少依赖: $c" >&2; exit 1; }
done
if [ ! -f "$PKG" ]; then
    echo "[-] 找不到加密压缩包: $PKG" >&2
    echo "    请把 install.sh 与 $PKG_NAME 放在同一目录，或用 PKG_URL 指定下载地址。" >&2
    exit 1
fi
...
if pgrep -x fpro-client >/dev/null 2>&1; then
    ...
    echo "[!] fpro-client 未运行，查看日志: tail -n 20 /var/log/fpro-client.log"
fi
...
if ssh -o StrictHostKeyChecking=accept-new -o ConnectTimeout=6 -o BatchMode=yes \
    ...
    echo "[*] 隧道回环自检未通过（进程可能还在握手），稍候请在本机执行："
